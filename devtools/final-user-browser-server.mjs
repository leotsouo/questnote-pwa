...
const databaseName = `QuestNoteTest-FinalUser-${instance}`;
...
const bootstrap = `(() => {
  ...
  Object.defineProperty(window, '__questNoteFinalTest', { value: Object.freeze({
...
const server = http.createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname === '/__final-user_test_guard__') {
      response.writeHead(200, { 'Content-Type': 'application/json' }).end(JSON.stringify({
        purpose: 'questnote-final-user-synthetic-only', instance, databaseName,
      }));
      return;
    }
    if (url.pathname === '/__final-user_db_guard__.js') {
      response.writeHead(200, { 'Content-Type': types['.js'] }).end(bootstrap); return;
    }
    const relative = decodeURIComponent(url.pathname).replace(/^\/+/, '') || 'devtools/final-user-browser-test.html';
    ...
    if (relative === 'index.html') {
      ...
      bytes = Buffer.from(html.replace('<head>', '<head>\n<script src="/__final-user_db_guard__.js"></script>'));
    }
...
server.listen(Number(process.argv[2] || 0), '127.0.0.1', () => {
  console.log(`Final user acceptance: http://127.0.0.1:${server.address().port}/devtools/final-user-browser-test.html`);
